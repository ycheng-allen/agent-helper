...
第三个标签页「玩命蹬」面向闲时免费额度场景：定义一个时间窗口（每日窗口如 00:00–08:00，支持跨午夜；或一次性窗口），排入最多 50 个任务，窗口开启后按设定并发执行——并行数 1 即串行，N（上限 6）为最多 N 个同时跑。窗口结束或手动停止时，在跑任务先 terminate、5 秒后强杀，剩余队列标记跳过；应用重启会自动恢复队列。
