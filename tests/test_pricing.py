...
class PricingTest(unittest.TestCase):
    ...
    def test_model_price_longest_prefix_and_markets(self):
        p = cmw.model_price("GLM-5.3-Flash")
        self.assertEqual(0.8, p["cny"][0])
        self.assertEqual(0.15, p["usd"][0])   # Z.ai international
        p = cmw.model_price("gpt-5.6-sol")
        self.assertIsNone(p["cny"])           # OpenAI 无国内牌价
        self.assertEqual(20.0, p["usd"][2])
        self.assertIsNone(cmw.model_price("totally-unknown-model"))

    def test_price_tokens_native_only(self):
        # glm-5.3: 2M fresh + 1M cached + 1M out → cny 8+1.6+28=37.6; usd 1.4*2+0.26+4.4=7.46
        cost = cmw.price_tokens("glm-5.3", 2_000_000, 1_000_000, 1_000_000)
        self.assertAlmostEqual(37.6, cost["cny"], places=2)   # fresh 1M*8 + cached 1M*1.6 + 28
        self.assertAlmostEqual(6.06, cost["usd"], places=2)   # intl: 1.4 + 0.26 + 4.4
        # openai: usd only, cached split
        cost = cmw.price_tokens("gpt-5.4", 1_000_000, 1_000_000, 1_000_000)
        self.assertAlmostEqual(0.25 + 15, cost["usd"], places=2)  # fresh=0
        self.assertNotIn("cny", cost)
        self.assertNotIn("cny", cost)

    def test_pricing_json_override(self):
        import json
        tmp = self.tmpdir().name
        json.dump({"rates": {"totally-unknown-model": {"in": 1, "out": 3, "currency": "usd"}}},
                  open(os.path.join(tmp, "pricing.json"), "w"))
        with patch.object(cmw, "APP_DIR", tmp):
            p = cmw.model_price("totally-unknown-model")
            self.assertEqual(1, p["usd"][0])

    def test_compute_cost_month_per_agent(self):
        tmp = self.tmpdir().name
        conn = cmw.db_connect(os.path.join(tmp, "state.db"))
        rows = [
            ("2026-09-01T01:00:00Z", "glm-5.3", "zcode", 1_000_000, 0, 1_000_000),
            ("2026-09-15T02:00:00Z", "gpt-5.4", "codex", 1_000_000, 0, 1_000_000),
            ("2026-08-20T03:00:00Z", "gpt-5.4", "codex", 1_000_000, 0, 1_000_000),
        ]
        ...
        self.assertAlmostEqual(36.0, z["cny"], places=1)          # domestic CNY
        self.assertAlmostEqual(36.0 / 7.1, z["usd_total"], places=1)  # EN display
        c = cmw.compute_cost(conn, "codex", cutoff)
        self.assertAlmostEqual(17.5, c["usd"], places=1)
        self.assertAlmostEqual(17.5 * 7.1, c["cny_total"], places=1)
        self.assertEqual(0.0, c["cny"])
    ...
    def test_api_data_cost_has_no_combined_fields(self):
        ...
        assert "month_total" not in data["cost"]
